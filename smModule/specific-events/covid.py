...
def best_perf():
    ...
    print(f"\r\033[KThe stock {best_perf_stock} has the best performance during the covid period : {best_perf} ({best_percentage}%)")
    print(f"\r\033[KThe stock {worst_perf_stock} has the worst performance during the covid period : {worst_perf} ({worst_percentage}%)")
...
def good_worst_day():
    ...
    print(f"\r\033[KThe {date_event} was the worst day for the US market (using the Dow Jones index) during the covid period, the stocks had an average lost of: ({res}%)")
